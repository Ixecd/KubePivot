...
func (r *StressRunner) Run() *StressResult {
	...
	case "s1":
		result = r.runS1(result)
	case "s2":
		result = r.runS2(result)
	case "s3":
		result = r.runS3(result)
	case "s4":
		result = r.runS4(result)
	case "s5":
		result = r.runS5(result)
	case "s6":
		result = r.runS6(result)
	default:
		result.Passed = false
...
func (r *StressRunner) runS1(result *StressResult) *StressResult {
...
func (r *StressRunner) runS2(result *StressResult) *StressResult {
	r.simulateBinPack(result)
...
func (r *StressRunner) runS3(result *StressResult) *StressResult {
	r.simulateBinPack(result)
...
func (r *StressRunner) runS4(result *StressResult) *StressResult {
...
func (r *StressRunner) runS5(result *StressResult) *StressResult {
	r.simulateBinPack(result)
...
func (r *StressRunner) runS6(result *StressResult) *StressResult {
	r.simulateBinPack(result)
	sim := NewCarbonSimulator("US-West")
...
// simulateBinPack 模拟 BinPack 计算（占位，v3.1 实际接 scheduler.BinPack()）.
func (r *StressRunner) simulateBinPack(result *StressResult) {
	...
	// v3.1 替换为: start := time.Now(); scheduler.BinPack(...); result.BinPackMs = time.Since(start).Milliseconds()

	// 占位值：假设调度器在万节点下内存 < 2GB
	// v3.1 替换为: var m runtime.MemStats; runtime.ReadMemStats(&m); result.MemoryMB = int64(m.Alloc >> 20)
