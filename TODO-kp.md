...
- [x] `kp scan` 独立命令：扫描所有服务镜像 CVE，支持 `--severity` 自定义阻断级别
- [x] `kp deploy` 自动集成 Trivy 扫描（有 trivy 才跑，没有静默跳过）
- [x] `kp doctor` 加 trivy 版本检查
- [x] `tools.mk` 加 `install.trivy` / `install.cosign`（跨平台官方脚本）
- [x] 实战验证：web3-blitz grpc CVE-2026-33186（CVSS 9.1）被扫出并修复

### v1.2.0 安全合规基线

- [x] `kp doctor` 安全检查（明文密码/Pod SecurityContext/RBAC 通配符/NetworkPolicy）
- [x] `kp init` 模板强化：NetworkPolicy + Pod SecurityContext + 资源 limits
- [x] `kp init` etcd chart 从 Deployment+emptyDir 升级到 StatefulSet+PVC
- [x] `kp init` 自动生成 `scripts/create-secret.sh`
- [x] `kp deploy` 前检查 secret 是否存在，缺失时警告提示
- [x] controller RBAC 最小权限（替换全权限）
- [x] `kp resume` bug 修复：ForceState(IDLE) 后重新部署，删除多余裸调用
...
- [x] `kp status` 展示每个 helm release 独立状态和 revision
- [x] `kp rollback` 按拓扑逆序逐层并行打印进度
- [x] `kp init --dry-run` 打印目录结构，不执行文件写入
...
- [x] `kp ai-plan`（Grok/Claude/OpenAI/豆包）
- [x] 统一进度输出（带时间戳和耗时）

### v0.8.x 及之前

- [x] `kp init` / `deploy` / `resume` / `rollback` / `release` / `down`
- [x] `kp doctor` / `status` / `history` / `diff`
