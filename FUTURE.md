...
| 编号 | 种子 | 状态 | 重新评估时机 |
|---|---|---|---|
| F1 | Cell-based Architecture (CBA) | 🌱 探索中 | v3.1 已完成 StatefulSet 迁移，v3.x 引入有状态资源保护时 |
...
- [ ] v3.1 Controller StatefulSet + etcd Learner 已落地（有状态基础设施就绪）
- [ ] 团队 / 工程容量能承担 O(N²) 测试矩阵
...
**v3.2+ 设计阶段（约 2026-Q3）** 重新评估：
- 那时候 v3.1 已上线（StatefulSet + etcd Learner），集群规模数据可见
- 有状态资源保护进入设计阶段
...
- v3.1 进展：Controller 已从 Deployment 迁移为 StatefulSet（etcd Learner sidecar），稳定 Pod 身份为 CBA 的 Cell 分配提供了基础设施前提。
- 触发因素：从工业 Matrix 工具的"细胞 grid"可视化得到启发
...
- 2026-05-05        更新至 v3.1 现状：StatefulSet 迁移已完成，版本号同步，重新评估时机调整为 v3.2+
