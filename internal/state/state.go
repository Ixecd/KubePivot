...
var validTransitions = map[State][]State{
	...
	StateValidating:   {StateRunning, StateRollingBack},
	StateRunning:      {StateInitializing, StateTerminated},
...
type DeployRecord struct {
	...
	IsFirst   bool      `json:"is_first"`   // true=首次部署，false=更新
...
func (m *Machine) canTransition(to State) error {
	...
	if !ok {
		return fmt.Errorf("未知状态: %s", m.record.State)
	}
	...
	return fmt.Errorf("非法状态转换: %s → %s", m.record.State, to)
}
